...
# Notificações
# =======================
notif-category-before-qualifying = Antes da Classificação
notif-category-qualifying-events = Eventos de Classificação
...
notif-quali-opens = <b>Classificação está aberta</b>
...
notif-quali-message = { $emoji } { $title }
    ...
    <i>Clique no botão '✅ Classificação Concluída' para desativar as notificações desta corrida</i>

notif-snooze-title = <b>Lembrete (adiado)</b>
...
notif-quali-message-disabled = { $emoji } { $title }
    ...
    <i>Clique no botão '🔄 Reativar' para reativar as notificações</i>

notif-quali-closed-title = <b>Classificação está fechada atualmente</b>

notif-quali-closed-message = { $emoji } { $title }

    🏁 <b>Corrida #{ $raceId }</b>
    📍 <b>{ $track }</b>
    ⏰ <b>Classificação fechada: { $qualiDeadline }</b>
    🏎 <b>Corrida: { $raceTime }</b>

    ⏳ <i>A classificação está fechada atualmente. A próxima sessão de classificação abrirá após a conclusão da corrida atual. Aguarde o cálculo da corrida.</i>
...
admin-calendar-update-failed = ❌ Não foi possível atualizar o calendário. Os dados existentes foram preservados; tente novamente mais tarde.
...
snooze-confirmed = 🔁⏰ Lembrete movido para { $time }
snooze-max-reached = 🔁❌ Limite de adiar atingido (3/3)
snooze-past-deadline = 🔁❌ Não é possível adiar após o prazo
snooze-past-next = 🔁❌ Próximo lembrete em { $minutes }min
...
validation-group-invalid-format = Formato de grupo inválido. Use: E, M1-5, P1-25, A1-80, R1-250
validation-group-e-no-numbers = Grupo Elite não tem grupos numerados (use 'E')
validation-group-range-m = Grupo Master deve ser 1-5
validation-group-range-p = Grupo Pro deve ser 1-25
validation-group-range-a = Grupo Amateur deve ser 1-80
validation-group-range-r = Grupo Rookie deve ser 1-150
...
button-website-mode = 🌐 Tipo de link: { $mode }
