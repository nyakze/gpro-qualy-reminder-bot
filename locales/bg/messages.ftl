...
notif-quali-opens = <b>Квалификацията е отворена (или ще се отвори скоро)</b>
...
notif-race-replay = 📺 <b>Повторение на състезание #{ $raceId } е налично</b>
    ...
    Ако състезанието вече е калкулирано, повторението е налично тук:
...
weather-temp-hum = Температура: { $temp }°C • Влажност: { $hum }%
...
weather-temp-hum-range = Температура: { $temp } • Влажност: { $hum }
...
notif-quali-results = 🏁 <b>Резултати от квалификацията - Състезание #{ $raceId }</b>
    ...
    📍 <b>{ $track }</b>
    ✅ <b>Квалификация приключена: { $qualiClose }</b>
    🏎 <b>Състезание: { $raceTime }</b>

    Резултати от квалификацията налични:

    🔗 <a href="{ $q12Link }">Класиране Кв1 и Кв2</a>
